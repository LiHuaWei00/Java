...
int currPage=1;
        ...
        List<Cart> list = ser.find(currPage);;
		request.setAttribute("list", list);
        int pages;  //总页数
        int count=ser.findCount(); //查询总记录数
        if(count%Cart.PAGE_SIZE==0){
            pages=count/Cart.PAGE_SIZE;
        }else{
            pages=count/Cart.PAGE_SIZE+1;
        }
        ...
        for(int i=1;i<=pages;i++){
            if(i==currPage){   //判断是否为当前页
                ...
                sb.append("<a href='Cartquery?page="+i+"'>"+i+"</a>"); //构建分页条
            }
            sb.append(" ");
        }
